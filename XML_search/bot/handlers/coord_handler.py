...
class CoordHandler(BaseHandler):
    ...
    async def handle_coordinates(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            # Обновляем метрики
            self._metrics.increment('coord_search_success')
            self._metrics.gauge('coord_search_results', len(results))
            
        except Exception as e:
            self._logger.error(f"Ошибка при обработке координат: {e}")
            self._metrics.increment('coord_handler_errors')
            await update.message.reply_text(
                self.formatter.format_error("Произошла ошибка при обработке координат.")
            )
